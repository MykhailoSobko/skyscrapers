...
def left_to_right_check(input_line: str, pivot: int) -> bool:
    ...
    count = 1
    highest = int(input_line[0])
    for i, _ in enumerate(input_line):
        if int(input_line[i]) > highest:
            highest = int(input_line[i])
            count += 1
    if count != pivot:
        return False
...
def check_uniqueness_in_rows(board: list) -> bool:
    ...
    Return True if buildings in a row have unique length, False
    otherwise.
